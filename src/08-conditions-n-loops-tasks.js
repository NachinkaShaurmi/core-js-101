...
function getFizzBuzz(num) {
  // return (num % 15) ? (num % 5) ? (num % 3) ? num : 'Fizz' : 'Buzz' : 'FizzBuzz';
...
function getCommonDirectoryPath(/* pathes */) {
  ...
  //   obj[i] = pathes[i].split('/');
  //   if (minLength > obj[i].length) minLength = obj[i].length;
  // }
  // if (minLength < 2) return answer;

  // for (let i = 0; i < minLength; i += 1) {
  //   for (let j = 0; j < pathes.length - 1; j += 1) {
  //     if (obj[j][i] !== obj[j + 1][i]) return answer;
  //     if (j === pathes.length - 1) answer += `${obj[i][j]}/`;
  //   }
  // }
  // return answer;
  throw new Error('Not implemented');
